...
export class SurcosService {
  ...
  console.log('Surco guardado:', surcoGuardado.id);
    console.log('Broker ID:', broker ? broker.id : 'Sin broker');
    console.log('Tópicos del broker:', broker ? broker.topicosAdicionales : 'N/A');

    if (broker && broker.topicosAdicionales && broker.topicosAdicionales.length > 0) {
      console.log('✅ Intentando crear sensores automáticos...');
      try {
        await this.mqttConfigService.crearSensoresParaTopicos(
          broker, 
          surcoGuardado.id, 
          broker.topicosAdicionales
        );
        console.log('✅ Sensores creados correctamente');
      } catch (error) {
        console.error('❌ Error creando sensores:', error);
      }
    } else {
        console.warn('⚠️ No se crearon sensores: El broker no tiene tópicos adicionales configurados.');
    }
    // --- FIN DEPURACIÓN ---
...
async sincronizarSensores(id: number): Promise<{ message: string, sensoresCreados: number }> {
    // 1. Buscar el surco con su broker
    const surco = await this.surcoRepository.findOne({
      where: { id },
      relations: ['broker', 'sensores'] // Traemos sensores para no duplicar si ya existen
    });

    if (!surco) throw new NotFoundException(`Surco #${id} no encontrado`);
    if (!surco.broker) throw new BadRequestException(`El Surco "${surco.nombre}" no tiene un Broker asignado`);
    
    const topicos = surco.broker.topicosAdicionales;
    if (!topicos || topicos.length === 0) {
      throw new BadRequestException(`El Broker "${surco.broker.nombre}" no tiene tópicos configurados`);
    }
