...
export class UsuariosService {
  ...
  async exportarExcel(): Promise<Buffer> {
    ...
    const usuariosFiltrados = usuarios.filter(
      (u) =>
        u.tipoUsuario &&
        rolesPermitidos.includes(u.tipoUsuario.nombre.toLowerCase()),
    );
  ...
  async exportarExcelFiltrado(filtros: any): Promise<Buffer> {
    ...
    if (filtros.searchTerm && filtros.searchTerm.trim() !== '') {
      ...
      usuarios = usuarios.filter((user) => {
        const nombreCompleto =
          `${user.nombre} ${user.apellidos || ''}`.toLowerCase();
    ...
    if (filtros.filterRol !== null) {
      usuarios = usuarios.filter(
        (user) => user.tipoUsuario?.id === filtros.filterRol,
      );
    }

    if (filtros.filterFicha !== null) {
      usuarios = usuarios.filter(
        (user) => user.ficha?.id_ficha === filtros.filterFicha,
      );
    }
  ...
  async cargarDesdeExcel(file: Express.Multer.File) {
    ...
    for (const [index, rawUsuarioData] of usuariosJson.entries()) {
      ...
      try {
        ...
        if (
          usuarioData.rol &&
          (usuarioData.rol.toLowerCase() === 'aprendiz' ||
            usuarioData.rol.toLowerCase() === 'pasante')
        ) {
        ...
        if (!rolesPermitidos.includes(usuarioData.rol.toLowerCase())) {
          throw new Error(
            `Rol no permitido: '${usuarioData.rol}'. Solo se permiten: ${rolesPermitidos.join(', ')}`,
          );
        }

        const rol = await this.tipoUsuarioRepository.findOne({
        ...
        if (
          ...
          if (!usuarioData['id ficha'] || usuarioData['id ficha'] === '') {
            throw new Error(
              `La ficha es obligatoria para el rol de ${usuarioData.rol.charAt(0).toUpperCase() + usuarioData.rol.slice(1)}.`,
            );
          }
          const ficha = await this.fichaRepository.findOne({
            where: { id_ficha: usuarioData['id ficha'] },
          });
          if (!ficha) {
            throw new Error(
              `La ficha con id_ficha '${usuarioData['id ficha']}' no existe.`,
            );
          }
        }
        ...
        if (existeCorreo) {
          throw new Error(
            `El correo '${usuarioData.correo}' ya está registrado.`,
          );
        }
        ...
        if (existeIdentificacion) {
          throw new Error(
            `La identificación '${usuarioData.identificacion}' ya está registrada.`,
          );
        }
        ...
        resultados.errores.push({
          fila: rowNum,
          usuario:
            usuarioData.correo || usuarioData.identificacion || 'Desconocido',
  ...
  async buscar(criterios: {
    ...
    if (criterios.nombre) {
      queryBuilder.andWhere(
        new Brackets((qb) => {
          qb.where('LOWER(usuario.nombre) LIKE LOWER(:nombre)', {
            nombre: `%${criterios.nombre}%`,
          }).orWhere('LOWER(usuario.apellidos) LIKE LOWER(:nombre)', {
            nombre: `%${criterios.nombre}%`,
          });
        }),
      );
    ...
    if (criterios.identificacion) {
      queryBuilder.andWhere(
        'CAST(usuario.identificacion AS TEXT) LIKE :identificacion',
        { identificacion: `%${criterios.identificacion}%` },
      );
    }

    if (criterios.rol) {
      queryBuilder.andWhere('LOWER(tipoUsuario.nombre) LIKE LOWER(:rol)', {
        rol: `%${criterios.rol}%`,
      });
    }
  ...
  async findAssignableUsers(): Promise<Usuario[]> {
    return this.usuarioRepository.find({
      relations: ['tipoUsuario', 'ficha'],
      where: {
        estado: true,
        tipoUsuario: {
          nombre: In(['Aprendiz', 'Pasante']),
        },
      },
      select: [
        'id',
  ...
  async buscarPorId(id: number) {
    ...
    const permisosRol = (usuario.tipoUsuario?.rolPermisos || []).map(
      (rp) => rp.permiso.nombre,
    );

    const permisosUsuario = (usuario.usuarioPermisos || []).map(
      (up) => up.permiso.nombre,
    );

    const permisosFinales = new Set<string>([
      ...permisosRol,
      ...permisosUsuario,
    ]);
  ...
  async actualizar(id: number, data: UpdateUsuarioDto): Promise<Usuario> {
    ...
    if (
      ...
      Number(data.identificacion) !== usuario.identificacion
    ) {
      const existeIdentificacion = await this.usuarioRepository.findOne({
        where: { identificacion: Number(data.identificacion), id: Not(id) },
      });
      if (existeIdentificacion) {
        throw new BadRequestException(
          `La identificación '${data.identificacion}' ya está registrada.`,
        );
      }
    }
    ...
    if (data.tipoUsuario) {
      const nuevoRol = await this.tipoUsuarioRepository.findOne({
        where: { id: data.tipoUsuario },
      });
      if (nuevoRol && nuevoRol.nombre.toLowerCase() === 'admin') {
        ...
        if (adminExistente && adminExistente.id !== id) {
          throw new BadRequestException(
            'Ya existe un administrador. No se puede asignar este rol.',
          );
        }
      }
    ...
    if (data.tipoUsuario) {
      ...
      if (
        nuevoRol &&
        nuevoRol.nombre.toLowerCase() !== 'aprendiz' &&
    ...
    if (data.id_ficha !== undefined) {
      if (data.id_ficha) {
        ...
        if (!ficha) {
          throw new BadRequestException(
            `La ficha con id_ficha ${data.id_ficha} no existe`,
          );
        }
        usuario.ficha = ficha;
      } else {
  ...
  async crear(data: CreateUsuarioDto): Promise<Usuario> {
    ...
    if (id_ficha) {
      const fichaEncontrada = await this.fichaRepository.findOne({
        where: { id_ficha },
      });
      if (!fichaEncontrada) {
        throw new BadRequestException(
          `La ficha con id_ficha ${id_ficha} no existe`,
        );
      }
  ...
  async solicitarRecuperacion(identificacion: string | number) {
    ...
    usuario.resetExpira = new Date(Date.now() + 60 * 60 * 1000);
    await this.usuarioRepository.save(usuario);
    ...
    return {
      success: true,
      message: 'Se ha enviado un link de recuperación al correo',
  ...
  async actualizarPerfil(id: number, data: UpdatePerfilDto): Promise<Usuario> {
    ...
    if (data.tipoIdentificacion !== undefined)
      usuario.Tipo_Identificacion = data.tipoIdentificacion;
