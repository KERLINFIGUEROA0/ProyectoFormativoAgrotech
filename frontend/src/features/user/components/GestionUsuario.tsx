    ...
    <div className="bg-white shadow-xl rounded-xl p-6 w-full h-full flex flex-col">
      <div className="flex-shrink-0">
        ...
        <div className="flex items-center justify-between mb-4">
          <input type="text" placeholder="Buscar por nombre, ID o rol..." className="border border-gray-300 rounded-lg px-4 py-2 w-full md:w-72 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      </div>
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} title={editingId ? 'Actualizar Usuario' : 'Información de Registro'}>
        <UserForm initialData={formInitialData} roles={roles} onSave={handleSave} onCancel={closeModal} editingId={editingId} />
      </Modal>
